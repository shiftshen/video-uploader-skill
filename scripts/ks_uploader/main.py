...
async def get_ks_cookie(account_file):
    ...
    async with async_playwright() as playwright:
        ...
        print("=== 快手登录 ===")
        print("请在手机上扫码登录...")
        await page.goto("https://studio.kuaishou.com/login")
        ...
        for i in range(120):
            ...
            # 检测登录成功 - 跳转到创作者中心
            if "studio.kuaishou.com" in current_url and "/login" not in current_url:
                print("✅ 检测到登录成功！")
                login_success = True
                break
            
            # 检测页面元素
            if await page.locator('text=上传作品').count() > 0:
                print("✅ 检测到登录成功！(发现上传作品)")
                login_success = True
                break
            
            if await page.locator('text=创作者中心').count() > 0:
                print("✅ 检测到登录成功！(发现创作者中心)")
                login_success = True
                break
